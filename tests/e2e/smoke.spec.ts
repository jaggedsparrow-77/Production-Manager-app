 ...
 *
 * Signed-in flows live in projects.spec.ts and require a seeded database.
 */

test("redirects an anonymous visitor to the login page", async ({ page }) => {
  await page.goto("/");
  await expect(page).toHaveURL(/\/login$/);
  await expect(page.getByRole("heading", { name: "Production Manager" })).toBeVisible();
});

test("guards the projects page", async ({ page }) => {
